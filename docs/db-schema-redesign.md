...
> **2026-09-12方針転換（GitHub issue #200）**: 本ドキュメントが前提とする「法人 = Clerk Organization（`organizations`/`organization_memberships`、複数メンバーで契約・月次上限を共有）」というモデルはVer1では実装を見送り、法人会員も個人と同じ「1アカウント＝1担当者」として`users`テーブルで扱う（`member_type`/`company_name`/`invoice_registration_number`列を追加）。以下は方針転換前に確定・実装した設計であり、歴史的経緯としてそのまま残す。現行のデータモデルは[[membership]]「個人と法人の関係」節を正とする。Ver2以降で複数メンバー対応を再導入する場合の移行手順も同節に記載している。
