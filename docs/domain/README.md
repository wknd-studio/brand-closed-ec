...
`order-fulfillment-flow.md`のみ「1ファイル＝1境界づけられたコンテキスト」の原則に従わない例外で、複数コンテキスト（[[ordering]]・[[procurement]]・[[fulfillment]]・[[returns]]）にまたがる注文確定後の流れをシーケンス図で一望するための補助資料。業務ルールの正はあくまで各コンテキストのファイル側にある。
