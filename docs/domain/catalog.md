...
- 要相談商品（個別見積もり、**2026-09-14: Ver1では扱わない、Ver2以降に延期**）・支払いタイミング（注文時払い／注文後払い）という商品属性
- 業者商品データインポート（CSV・スクレイピング）による商品情報の継続的な更新の位置づけ
- ~~手配リクエスト（カタログ掲載外商品の取り寄せ依頼）~~（2026-09-21: 実装しないことを確定。後述）
...
| 用語                                                 | 定義                                                                                                                                                                                                                                                                                                                                     |
| ---------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **要相談商品（2026-09-14: Ver1では扱わない）**       | 価格が個別見積もりとなる商品（`is_negotiable`）。ランク別価格を持たず、後払いフローでのみ扱われる想定だったが、価格が注文確定時点で分からないことに起因する設計上の複雑さ（月次仕入れ上限チェックのタイミング等）が大きいため、Ver1では実装せずVer2以降に延期する。Ver1では全商品が固定価格（`is_negotiable=false`）である前提で設計する |
| **支払いタイミング**                                 | 商品ごとに設定する決済方式（`payment_timing`）。注文時払い（`at_order`、Stripe Checkoutで即時決済）と注文後払い（`after_order`、運営者が任意のタイミングで請求書発行）の2種類。いずれも価格は注文確定時点で確定している（Ver1では要相談商品を扱わないため）                                                                              |
...
| **手配リクエスト（2026-09-21実装しないことを確定）** | ~~カタログ掲載外の商品の取り寄せ依頼機能~~。メーカー側が御社（卸業者）に提供できる商品自体があらかじめ決まっており、カタログに無い商品を個別に仕入れられる余地がそもそも無いため、Ver1・Ver2以降を問わず実装しない                                                                                                                       |
...
- **要相談商品（`is_negotiable=true`）はVer1では作成できない**（2026-09-14）。カタログ管理画面で`is_negotiable`を選択できるようにするのはVer2以降。Ver1は固定価格商品（`is_negotiable=false`）のみを前提に、上記のランク別価格入力必須ルールを全商品に適用する
...
- ✅ ~~手配リクエスト機能そのものが未実装~~（2026-09-21解決）: 実装しないことを確定。メーカー側が御社に提供できる商品ラインナップ自体が固定されており、カタログ外の商品を個別に仕入れる余地が実務上存在しないため。`docs/glossary.md`の用語定義も削除した
- ✅ ~~数量ベースの在庫管理・ソフト予約は未実装~~（2026-09-14解決・該当なしと判明）: `docs/glossary.md`の「在庫数」「ソフト予約」という記述は、旧5ランクモデル時代の前提を引きずった古い記述だった。御社は無在庫転売型モデル（会員の注文後に運営者が仕入れ先へ発注する）であり、そもそも自社で在庫を持たないため、数量管理・ソフト予約・複数会員間の在庫競合制御という概念自体が成立しない。現行の3値`availability`（`available`/`out_of_stock`/`discontinued`）のみで正しく、`docs/glossary.md`側を更新する必要がある
...
- ✅ ~~商品一覧のカテゴリ絞り込み・並び替え・検索の詳細仕様~~（2026-09-21解決）: 最小要件（ブランド一覧・ブランドごとの商品一覧・商品名/ブランドでの条件検索）を確定。GitHub issue #238として実装予定。UIの詳細は他機能の実装後、画面を見て判断する
- 🔲 **お気に入り一覧の表示項目・ランク制限で閲覧不能になった商品の扱い**: GitHub issue #239として実装予定だが、表示項目・ランク制限商品の扱いの具体的な仕様は実装時に確定させる
