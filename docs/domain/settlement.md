...
- `checkout`決済単位の作成時・`invoice`決済単位の作成時（＝運営者が請求作成を実行した瞬間）にも、それぞれの決済単位に含まれる明細の合計金額で同じ判定を行う。ただし`after_order`の未請求明細は`settlement_id`が`NULL`のままでもすでに「今月確定している金額」に計上済み（上記）のため、請求作成はその金額を新たに追加するのではなく、すでに数えられている金額をInvoiceという形に変換するだけである。したがって**行ロックによる直列化（下記）を前提にすれば、請求作成時の判定は注文確定前のチェックと常に同じ結果になり、通常運用で新たに超過が判明することはない**
- **同一顧客の同時注文による競合の防止・完全排除（2026-09-21確定）**: `after_order`の未請求明細も「今月すでに確定している金額」に含める設計（上記）により、通常の順次処理では前の注文の未請求分を後続の注文が正しく考慮できる。残っていたのは、全く同じ瞬間に2つの注文（または注文と請求作成）が処理される真の競合（レースコンディション）のみだったため、月次上限の計算〜判定〜保存を行うトランザクションの冒頭で、対象ユーザー（または組織）の行に`SELECT ... FOR UPDATE`の行ロックをかけて直列化した。これにより、その残っていた競合も含めて構造的に閉じている。新しいテーブル・概念は不要で、既存のトランザクションへの数行の追加で対応できる（`place-order`・「請求作成」の両方に適用。issue #220・#224）
...
**`limit_exceeded`からの回復（2026-09-14追記、2026-09-21改訂: 行ロック導入により正常系では原則発生しない状態と整理）**

2026-09-21に追加した行ロック（上記）により、`limit_exceeded`に至る経路として文書上想定していた競合は構造的に閉じている。価格が固定である（要相談商品はVer1で扱わない）ことと、後払いの未請求分も確定済み金額に含める設計を組み合わせると、注文確定前のチェックを通過した時点で、その後の請求作成時のチェックも必ず通過する。したがって、**通常運用で`limit_exceeded`が発生することは想定していない**。

それでも次のステータス・回復手順は削除せず残す。理由は、実装上のバグ（チェック漏れ・集計ロジックの誤り等）やデータ不整合が万一発生した場合に、金額超過の請求書がそのままStripeへ送付されてしまう事態を防ぐための安全網（フェイルセーフ）としての意味があるため。「頻繁に発生する正常な業務フロー」ではなく「起きてはならないことが起きた場合の受け皿」という位置づけである。
...
- ✅ ~~返金時の決済単位の扱い~~（2026-09-20解決）: 既存の`cancelled`を再利用する。詳細な返金記録は[[returns]]の`returns`テーブル側が正
- 🔲 **請求書の未発行放置リスクへの対策**: 自動発火をやめたことで「入荷したのに請求し忘れる」リスクが生じる。対応待ち一覧に「入荷済みだが未請求」のフィルタ表示を用意する等の運用上のケアが必要（新しいロジックではなく既存データのフィルタ表示で足りる想定。issue #224で実装時に対応）
