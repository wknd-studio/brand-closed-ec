...
- [x] T022 [P] `quickstart.md` の全シナリオをステージング環境で手動検証する（ユーザーが手動確認済み）
- [x] T023 [P] `pnpm test:e2e` で登録〜カタログ〜注文確定のE2Eを実行する（specs/003-checkout-invoice-e2eでカタログ〜チェックアウトのE2E（`checkout.spec.ts`・`invoice.spec.ts`）を実装・マージ済みのため実行可能になった。ローカルで`doppler run -- npx playwright test --project=chromium --workers=1`を実行し、全12件（setup・registration・login・onboarding・checkout×3・invoice）が成功することを確認した）
