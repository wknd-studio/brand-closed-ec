...
- 上記の議論の延長で、`vendor`という単一ドキュメント型（業務的情報＋技術的取り込み設定を同居）自体を見直した。ブランドは行ごとのデータであり業者/データソース単位で決め打ちできないこと、CSVは運営者がデータだけで完結できる一方スクレイピングは開発者のコード実装を伴うためStudio上での操作可否を型で分けたいこと、の2点から`vendor`を廃止し`csvCatalog`（運営者管理）・`scrapingCatalog`（開発者管理、Studio上で作成・削除をロック）に再設計した。詳細はdata-model.md「商品データソース」参照。
