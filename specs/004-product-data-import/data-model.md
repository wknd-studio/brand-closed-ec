...
スクレイピング対象の商品データソース1本の設定。開発者がスクレイピングコード（`scripts/product-import/vendors/<scrape_adapter_id>/scraper.ts`）を実装する際に、コード側（シードスクリプト等）で一緒に用意する。Studio上では文字通りの「定数」として扱い、運営者は新規作成・編集・削除のいずれもできず閲覧のみとする（ユーザーとの協議：「スクレイピングの設定は定数かつ削除できないようにした方がいい」）。`document.actions`（`publish`・`delete`・`duplicate`をブロック）・`document.newDocumentOptions`（全コンテキストで新規作成をブロック）に加え、全フィールドを`readOnly: true`にする二重の防御を行っている（UIレベルの誤操作防止であり、データセットへの書き込み権限を持つ場合はAPI経由で操作できてしまう点に注意）。内容の変更が必要な場合は開発者がコードで更新する。運営者は実行結果（インポート実行結果・要確認キュー）の確認のみに関わる。
