...
1. サンプルCSV（`src/lib/product-import/__fixtures__/vendor-a-sample.csv`、業者Aの列構成を模したもの、50行）を用意する
2. Sanity Studioの「商品管理」→「CSVインポート」ツールを開き、サンプルCSVをアップロードする
3. **期待結果**: 書き込み前に「成功見込み: 50件、エラー見込み: 0件」の検証プレビューが表示される
4. 「実行を確定する」を押す
5. **期待結果**: Sanity上に50件の`product`ドキュメントが新規作成され、`jan_code`・`source_vendor`が正しく設定されている
6. 同じCSVをもう一度アップロードして実行する
7. **期待結果**: 既存の50件が更新され、重複した商品としては作成されない（`ProductImportRun`の`success_count`は50、Sanity上の`product`件数は変わらない）
...
```bash
pnpm tsx scripts/product-import/run-on-demand.ts --vendor=vendor-b --dry-run
```

1. **期待結果**: `vendor-b`のスクレイピングアダプターが実行され、統一データ形式のレコードが標準出力に表示される（`--dry-run`のためSanityへの書き込みはしない）
2. `--dry-run`を外して再実行する
3. **期待結果**: Sanity上に業者Bの商品が作成され、`ProductImportRun`（`triggered_by: "on_demand"`）が記録される

## User Story 2: オンデマンド実行（Studio経由）

1. Sanity Studioの業者一覧から業者Bを開き、「今すぐ実行」ボタンを押す
...
1. 業者Bのスクレイピング対象データから、商品を1件テスト用に取り除いた状態を用意する
2. 定期実行スクリプトを2回連続で実行する（1回目: 全件あり、2回目: 1件除いた状態）

   ```bash
   pnpm tsx scripts/product-import/run-scheduled-sync.ts --vendor=vendor-b
   ```
...
- `csv-adapter.test.ts`: 業者別列マッピングの変換
- `vendors/<vendor-id>/scraper.test.ts`: 固定HTMLフィクスチャからの統一データ形式への変換
