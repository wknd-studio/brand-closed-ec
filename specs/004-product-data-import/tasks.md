...
- [x] T032 [US3] `run-scheduled-sync.ts`を実装する（`scripts/product-import/vendors/`配下の`__`接頭辞を除く全アダプターディレクトリを処理、catalog単位の失敗（アダプター読み込み失敗・スクレイプ失敗）はスキップして他catalogの処理を継続しつつ`productImportRun`に`outcome: "failed"`として記録、成功時はCSV化してSanity file assetへアップロードし対象`csvCatalog`の`pending_csv`フィールド（`source: "scheduled_scrape"`）へ保存する。**商品データへの直接書き込み（apply-import呼び出し）は行わない**——書き込み前の人間による確認を、定期実行でも必ず担保するため（Phase4で確立した方針をPhase5にも適用。ユーザーとの協議）。FR-009, FR-010, FR-012）: `scripts/product-import/run-scheduled-sync.ts`, `src/lib/product-import/filter-adapter-directories.ts`（依存: T022, T013）
- [x] T032a [US3] （追加、Phase5完了後の設計変更）`productCsvUpload`ドキュメント型を廃止し、`csvCatalog.pending_csv`フィールドへ統合する。理由: 要確認キュー機能（T029〜T031, T034〜T035）の廃止により、カタログをまたいだ複数時点のCSVの経時比較が不要になったため、「1カタログにつき保留中CSVは最大1件」という前提で単純化できると判断した（ユーザーとの協議）: `src/sanity/schemas/csv-catalog.ts`, `src/sanity/schemas/product-csv-upload.ts`（削除）, `src/lib/product-import/csv-upload.ts`, `src/sanity/tools/product-import/product-import-tool.tsx`, `scripts/product-import/run-scheduled-sync.ts`（依存: T032）
