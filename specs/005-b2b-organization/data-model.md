...
| `profile_completed_at` | TIMESTAMPTZ \| null      | 氏名・電話番号の入力が完了した日時。当初は既存会員の遡及対応（FR-020）の判定に使う想定だったが、PR #143でFR-020自体がスコープ外になったため、現時点ではどの処理からも設定・参照されない未使用カラム（将来FR-020を実装する際に利用する） |
...
**プロフィール入力（FR-019）**: 個人会員は`/onboarding/plan`、法人代表者は`/onboarding/organization`（代表者名を姓・名に分割した入力欄）で、それぞれのオンボーディング画面内で氏名・電話番号を収集する。法人代表者はセルフサインアップした本人であるという前提のもと、組織作成時に入力された代表者の氏名・電話番号を、そのまま本人の`users.first_name` / `last_name` / `phone_number`にも反映する（`create-organization.ts`）ため、別画面での二重入力は発生しない。

**スコープ外（FR-020、PR #143）**: 当初案にあった、既存会員の氏名・電話番号未入力を`middleware.ts`のゲートで検知し`/profile/complete`相当の画面へリダイレクトする仕組みは、本機能が本番未リリースで対象となる既存会員が実在しないため実装しない。将来的に既存会員が発生した状態で遡及対応が必要になった場合は、この`profile_completed_at`カラムを用いて別途仕様化する。
