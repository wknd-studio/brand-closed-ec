  ...
  },
  {
    ...
    body: (
      <Stack space={3}>
        ...
        <Card padding={3} radius={2} tone="caution">
          <Text size={1}>
            同じCSVを再度インポートすると、JANコード（無ければ商品名+ブランド名）が一致する既存商品は業者データ由来の項目（定価・在庫状況・仕入れ掛け率・入数等）が上書き更新されます。一方、支払いタイミング・ランク別仕入れ価格・最低閲覧ランクは新規作成時にだけ既定値が設定され、更新時は運営者が手動調整した値を上書きしません。
          </Text>
        </Card>
      </Stack>
    ),
