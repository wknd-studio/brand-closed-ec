      ...
      <Stack space={4}>
        ...
        <SubSection label="手順B: CSVを取り込む（毎回の作業）">
          <StepList>
            ...
            <StepWithNote note="保留中のCSVがあれば「この保留中のCSVを使う」ボタンが表示される（「4. 保留中のCSVについて」参照）">
      ...
      <Stack space={4}>
        ...
        <Card padding={4} radius={2} tone="primary">
          <Stack space={2}>
            ...
            <Text size={1}>
              「3.
              CSVで一括インポートする」の手順Bでカタログを選ぶと、保留中のCSVがあれば「この保留中のCSVを使う」ボタンが表示されます。そこから検証プレビュー→確定の操作を行って初めて商品データに反映されます。無人実行の結果をそのまま反映しないための仕組みです。確定すると保留は解除されます。
            </Text>
          </Stack>
        </Card>
      </Stack>
